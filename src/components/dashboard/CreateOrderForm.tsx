...
const CreateOrderForm: React.FC<CreateOrderFormProps> = ({ onSubmit, isPending, onCancel, items, buttonText = "অর্ডার তৈরি করুন" }) => {
  ...
  const parseSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      paymentMethod: formData.paymentStatus === "PAID" ? "ONLINE" : "COD",
      deliveryAddress: {
  ...
  return (
   <form onSubmit={parseSubmit} className="space-y-6 max-h-[75vh] overflow-y-auto px-1">
      ...
      <div className="pt-4 flex justify-end gap-2 border-t mt-6">
          <Button type="button" variant="outline" className={"rounded-md hover:bg-primary/10"} onClick={onCancel}>বাতিল</Button>
          <Button type="submit" className="bg-fire text-white font-semibold hover:bg-fire-dark" disabled={isPending || orderItems.length === 0}>
              {isPending ? "তৈরি হচ্ছে..." : buttonText}
          </Button>
      </div>
    </form>
  )
