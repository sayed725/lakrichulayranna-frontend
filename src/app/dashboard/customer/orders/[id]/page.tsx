...
import { ArrowLeft, MapPin, CreditCard, PackageCheck, AlertCircle, Clock, Check, Phone, User, FileText, Copy } from "lucide-react";
  ...
  return (
    ...
    >
      <motion.div variants={fadeInUp}>
        <Link href="/dashboard/customer/orders" className="inline-flex items-center text-sm font-bold font-bengali text-muted hover:text-fire mb-6 transition-colors">
          ...
          <motion.div variants={fadeInUp}>
            ...
            <div className="text-sm bg-cream/30 p-4 rounded-2xl border border-border leading-relaxed space-y-2 font-bengali">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-charcoal">{parsedAddress.fullName || order.user?.name || 'N/A'}</span>
                {(parsedAddress.fullName || order.user?.name) && (
                  <button
                    onClick={() => handleCopy(parsedAddress.fullName || order.user?.name, "fullName", "নাম")}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-latin text-charcoal">{parsedAddress.phone || order.user?.phone || 'N/A'}</span>
                {(parsedAddress.phone || order.user?.phone) && (
                  <button
                    onClick={() => handleCopy(parsedAddress.phone || order.user?.phone, "phone", "ফোন নাম্বার")}
                    className="text-muted hover:text-fire transition-colors p-1 rounded hover:bg-cream-dark/50 ml-auto cursor-pointer"
